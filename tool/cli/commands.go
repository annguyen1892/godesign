...
func RegisterCommands(app *cobra.Command, c *client.Client) {
	...
	command = &cobra.Command{
		Use:   "create",
		Short: `creates a review`,
	}
	tmp1 := new(CreateReviewCommand)
	sub = &cobra.Command{
		Use:   `review ["/reviews/"]`,
		Short: `review`,
		Long: `review
...

{
   ...
   "title": "ae9l6"
	...
	tmp2 := new(ShowReviewCommand)
	sub = &cobra.Command{
		Use:   `review ["/reviews/ID"]`,
		Short: `review`,
...
func (cmd *CreateReviewCommand) Run(c *client.Client, args []string) error {
	...
	var payload client.ProductReviewPayload
	if cmd.Payload != "" {
...
func (cmd *ShowReviewCommand) Run(c *client.Client, args []string) error {
	...
	if len(args) > 0 {
		...
		path = fmt.Sprintf("/reviews/%v", cmd.ID)
	}
	logger := goa.NewLogger(log.New(os.Stderr, "", log.LstdFlags))
	ctx := goa.WithLogger(context.Background(), logger)
...
func (cmd *ShowReviewCommand) RegisterFlags(cc *cobra.Command, c *client.Client) {
	var id int
	cc.Flags().IntVar(&cmd.ID, "id", id, `Review ID`)
}
